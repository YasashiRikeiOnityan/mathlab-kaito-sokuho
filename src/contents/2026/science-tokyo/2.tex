...
\setlength{\defaultmathindent}{\mathindent}
\setlength{\mathindent}{2mm}
\begin{align*}
    U=\{(x, y, z) ~|~ x+y+z=3n,~ x,y,z \text{は正の整数}\}
\end{align*}
\setlength{\mathindent}{\defaultmathindent}
\noindent
このとき
\begin{align*}
...
\begin{enumerate}
    ...
    \item [\tokeini] $x, y, z$ のうち，2つのみが同じ値の場合 \\
        $x=y\neq z$ のときを考える．
        \begin{enumerate}
            \item [\maruichi] $n$ が奇数のとき \\
                $x$ のとり得る値は $1$ から $(3n-1)/2$ までであり，\tokeiichi の場合を
                除くととり得る点の個数は $(3n-1)/2-1=(3n-3)/2$ 個である．
            \item [\maruni] $n$ が偶数のとき \\
                $x$ のとり得る値は $1$ から $(3n-2)/2$ までであり，\tokeiichi の場合を
                除くととり得る点の個数は $(3n-2)/2-1=(3n-4)/2$ 個である．
        \end{enumerate}
