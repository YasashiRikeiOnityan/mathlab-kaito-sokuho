...
\noindent
\kakkoni\quad $k=3, 4, \ldots , n-1$ に対し $x+y+z=k$ をみたす $(x, y, z)$ の個数は，$k$ 個のボールの間に2本の仕切りを入れる選び方と等しいので，
$\kumiawase{k-1}{2}$ 個である．よって，求める個数は
\begin{align*}
...
\end{align*}
である．ここで，$x, y, z$ がすべて異なる事象の余事象を考える．

\begin{enumerate}
    \item [\tokeiichi] $x=y=z$ の場合 \\
        $(x, y, z) = (n, n, n)$ の1個
    \item [\tokeini] $x, y, z$ のうち2つが同じ値の場合 \\
        $x=y\neq z$ のときを考える．
        \begin{itemize}
            \item $n$ が奇数のとき \\
        ...
        \end{itemize}
        $y=z\neq x, z=x\neq y$ の場合も同様に計算できるので，$n$ が奇数のとき $3(3n-3)/2$ 個，$n$ が偶数のとき $3(3n-4)/2$ 個である．
\end{enumerate}
...
\noindent
\tokeiichi , \tokeini より，$x, y, z$ の3つの座標がすべて異なる場合の数 \astab は， 
$n$ が奇数のとき，
\begin{align*}
...
\columnbreak
\noindent
$x<y<z$ をみたす場合の数は \astab からさらに $1/6$ になるので，
$n$ が奇数のとき，
\begin{align*}
    (9n^{2}-18n+9)/12=\boldsymbol{(3n^{2}-6n+3)/4} ~\text{\bf 個}
\end{align*}
$n$ が偶数のとき，
\begin{align*}
