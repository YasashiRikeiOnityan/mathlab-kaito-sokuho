...
$x^{2}+p^{\prime}x+q^{\prime}=0$ が成り立つとすると，
\begin{align*}
    (p-p^{\prime})x+(q-q^{\prime})=0~\sdots~\astab
\end{align*}
を得る．ここで $p-p^{\prime}\neq 0$ と仮定する．$\astab$ の両辺を $p-p^{\prime}$ で割ると $x=-\bunsuu{q-q^{\prime}}{p-p^{\prime}}$ 
...
$x^{3}+p^{\prime}x^{2}+q^{\prime}x+r^{\prime}=0$ が成り立つとすると，
\begin{align*}
...
\columnbreak
\noindent
ここで $p-p^{\prime}\neq 0$ と仮定する．$\astab$ の両辺を $p-p^{\prime}$ で割ることで，$A, B$ を有理数として $x^{2}+Ax+B=0$ の形で表せる．
また，
\begin{align*}
    x^{2}+(-2a+C)x+(a^{2}-Ca+D)=0
\end{align*}
が成り立つ有理数 $C, D$ をとることができ，$y=x-a$ とすると
\begin{align*}
    y^{2}+Cy+D=0 \Y y^{2}=-Cy-D
\end{align*}
となる．$y=x-a=b\sqrt[3]{n}$ より，$y^{3}=b^{3}n$ である．これに $y^{2}=-Cy-D$ を代入して次数を下げると
\begin{align*}
    y(-Cy-D)&=b^{3}n \\
    -Cy^{2}-Dy&=b^{3}n \\
    -C(-Cy-D)-Dy&=b^{3}n \\
    (C^{2}-D)y+(CD-b^{3}n)&=0
\end{align*}
$x$ が無理数であることから $\sqrt[3]{n}$ は無理数であり，$y$ も無理数となるから $C^2=D$ かつ $CD-b^{3}n=0$ を得る．$D=C^{2}$ を $CD-b^{3}n=0$ に代入すると，
\begin{align*}
    C^{3}-b^{3}n=0 \Y \sqrt[3]{n}=\bunsuu{C}{b}
\end{align*}
右辺は有理数であるが $\sqrt[3]{n}$ が無理数であることに矛盾する．
したがって，$p=p^{\prime}$ であり，これを $\astab$ に代入した
\begin{align*}
    (q-q^{\prime})x+(r-r^{\prime})=0
\end{align*}
については \kakkoichi と同様の議論から $q=q^{\prime}, r=r^{\prime}$ を得る．
よって，$p, q, r$ の一意性が示された．\owari
